...
angular.module('ngShowcaseApp').factory('industryTreeData', function() {
  /**
  ...
  function TreeData(tree, cbIsSame) {
     ...
     * @returns {boolean}
     */
    this.isFolded = function(item,boole) {
      if(boole == true){
        return item.folded;
      }else {
         return !item.folded;
    ...
    this._updateParents = function(items, item) {
      _.each(items, function(subItem) {
        if(_this.hasChildren(subItem, item)) {
          ...
          _this._updateParents(subItem.children, item);
    ...
    this._find = function(items, item) {
      ...
      for (var i = 0; i < items.length; ++i) {
        ...
        if (this.isSame(subItem, item))
          return subItem;
        // 否则递归查找
        var subResult = _this._find(subItem.children, item);
        if (subResult)
          return subResult;
      }
      return null;
    ...
    this.hasChildren = function(parent, subItem) {
      ...
      return !!_.find(subItems, function(subItem) {
        return _this._find(parent.children, subItem);
      });
    };
    /**
     * 选中节点
